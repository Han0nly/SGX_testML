...
int encrypt_file(int DO_ID, int file_num)
{
	...
	sprintf(buf1, "DataFiles/DO%d_%d.txt", DO_ID, file_num);
...
int enclave_compute_task1(enclave_info_t* enclave_info, int DO_ID, int file_num)
{
	...
	sprintf(buf1, "data/DO%d_%d_en.txt", DO_ID, file_num);
	sprintf(buf2, "data/DO%d_%d_en_mac.txt", DO_ID, file_num);
	sprintf(buf3, "data/DO%d_%d_result.txt", DO_ID, file_num);
	sprintf(buf4, "data/DO%d_%d_result_mac.txt", DO_ID, file_num);
	sprintf(buf5, "data/DO%d_%d_result_rand_key_mac.txt", DO_ID, file_num);
	...
	ret = ECALL_compute_task1_single(
...
//            result_gcm_mac);
    ...
    {
//        fprintf(OUTPUT, "\nError, call ECALL_compute_task%d fail [%s]. Session closed.\n", Request_operation, __FUNCTION__);
        fprintf(OUTPUT, "\nError status:0x%X\n",ret);
        fprintf(OUTPUT, "\nError return:0x%X\n",status);
        // return 1;
    }
...
/* Execute the computing task for multiple files*/
int enclave_compute_task(enclave_info_t* enclave_info, int Request_DC, int Request_type, int Request_start, int Request_end, int Request_operation)
{
    ...
    FILE *ofp_result = fopen(buf3, "wb");
    FILE *ofp_result_mac = fopen(buf4, "wb");
    if(ofp_result==NULL||ofp_result_mac==NULL){
        fprintf(OUTPUT, "\nFile %s or %s not exist.\n",buf3,buf4);
        return 1;
    }
    ...
    for(i = Request_start; i <= Request_end; i++)
    {
        ...
        switch(Request_type)
        {
            case 0:
            {
                sprintf(buf1, "DataFiles/DO1_%d.txt", i);
        ...
        case 2:
            printf("\nBegin task 2 [training SVM classifier].\n");
             ret = ECALL_compute_task2(
                             ...
                             &status,
                             enclave_info->context,
                             num_data,      // uint8_t* p_data_encrypted,
                             ctextSizes,    // uint32_t data_size,
                             macSizes,      // uint8_t* p_data_gcm_mac
                             data,          // uint8_t* p_result_encrypted
                             ctextSizes_sum,// uint32_t result_size
                             data_mac,      // uint8_t* p_result_gcm_mac
                             macSizes_sum,
                             result_encrypted,
                             result_size,
                             result_gcm_mac);
...
int enclave_compute_task_normal(int Request_type, int Request_start, int Request_end, int Request_operation)
{
    ...
    FILE *ofp_result = fopen(buf3, "wb");
    FILE *ofp_result_mac = fopen(buf4, "wb");
    if(ofp_result==0||ofp_result_mac==0){
        fprintf(OUTPUT, "\nFile %s or %s not exist.\n",buf3,buf4);
        return 1;
    }
    ...
    switch(Request_operation)
    {
        ...
        case 2:
        {
            printf("\nBegin task 2 [training SVM classifier].\n");
            /* Get the problem */
            read_svm_problem_from_plaintext(g_data, ctextSizes_sum, N_sum, K, C);
